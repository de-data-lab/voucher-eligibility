...
```{r, echo=FALSE}
hud_data <- read_csv(here('data/processed/hud_2020_tract_joined.csv'))
...
hud_de <- hud_data %>% filter(str_detect(entities,"DE"))
hud_de <- hud_de %>% dplyr::rename(GEOID=code)
de_summary_table <- hud_de %>% 
    group_by(GEOID) %>% 
    summarize(tot = sum(number_reported))
```

```{r, echo=FALSE}
eligible <- rent %>% inner_join(rent30, by="GEOID") %>% filter(med_rent_percE > 30)
```

```{r, echo=FALSE}
de_h30 <- hud_de %>% group_by(GEOID) %>% 
